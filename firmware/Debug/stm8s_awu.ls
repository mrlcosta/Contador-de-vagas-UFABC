  ...
  49  0020 0f            	dc.b	15
  50  0021 0f            	dc.b	15
  79                     ; 67 void AWU_DeInit(void)
  79                     ; 68 {
  81                     	switch	.text
  82  0000               _AWU_DeInit:
  86                     ; 69     AWU->CSR = AWU_CSR_RESET_VALUE;
  88  0000 725f50f0      	clr	20720
  89                     ; 70     AWU->APR = AWU_APR_RESET_VALUE;
  91  0004 353f50f1      	mov	20721,#63
  92                     ; 71     AWU->TBR = AWU_TBR_RESET_VALUE;
  94  0008 725f50f2      	clr	20722
  95                     ; 72 }
  98  000c 81            	ret
 260                     ; 82 void AWU_Init(AWU_Timebase_TypeDef AWU_TimeBase)
 260                     ; 83 {
 261                     	switch	.text
 262  000d               _AWU_Init:
 264  000d 88            	push	a
 265       00000000      OFST:	set	0
 268                     ; 86     assert_param(IS_AWU_TIMEBASE_OK(AWU_TimeBase));
 270                     ; 89     AWU->CSR |= AWU_CSR_AWUEN;
 272  000e 721850f0      	bset	20720,#4
 273                     ; 92     AWU->TBR &= (uint8_t)(~AWU_TBR_AWUTB);
 275  0012 c650f2        	ld	a,20722
 276  0015 a4f0          	and	a,#240
 277  0017 c750f2        	ld	20722,a
 278                     ; 93     AWU->TBR |= TBR_Array[(uint8_t)AWU_TimeBase];
 ...
 286                     ; 96     AWU->APR &= (uint8_t)(~AWU_APR_APR);
 288  0027 c650f1        	ld	a,20721
 289  002a a4c0          	and	a,#192
 290  002c c750f1        	ld	20721,a
 291                     ; 97     AWU->APR |= APR_Array[(uint8_t)AWU_TimeBase];
 ...
 302  003c 84            	pop	a
 303  003d 81            	ret
 358                     ; 108 void AWU_Cmd(FunctionalState NewState)
 358                     ; 109 {
 359                     	switch	.text
 360  003e               _AWU_Cmd:
 364                     ; 110     if (NewState != DISABLE)
 366  003e 4d            	tnz	a
 367  003f 2706          	jreq	L331
 368                     ; 113         AWU->CSR |= AWU_CSR_AWUEN;
 370  0041 721850f0      	bset	20720,#4
 372  0045 2004          	jra	L531
 373  0047               L331:
 374                     ; 118         AWU->CSR &= (uint8_t)(~AWU_CSR_AWUEN);
 ...
 437                     ; 135 void AWU_LSICalibrationConfig(uint32_t LSIFreqHz)
 437                     ; 136 {
 438                     	switch	.text
 439  004c               _AWU_LSICalibrationConfig:
 441  004c 5206          	subw	sp,#6
 442       00000006      OFST:	set	6
 445                     ; 138     uint16_t lsifreqkhz = 0x0;
 447                     ; 139     uint16_t A = 0x0;
 449                     ; 142     assert_param(IS_LSI_FREQUENCY_OK(LSIFreqHz));
 451                     ; 144     lsifreqkhz = (uint16_t)(LSIFreqHz / 1000); /* Converts value in kHz */
 ...
 463                     ; 148     A = (uint16_t)(lsifreqkhz >> 2U); /* Division by 4, keep integer part only */
 ...
 508  0097 81            	ret
 531                     ; 165 void AWU_IdleModeEnable(void)
 531                     ; 166 {
 532                     	switch	.text
 533  0098               _AWU_IdleModeEnable:
 537                     ; 168     AWU->CSR &= (uint8_t)(~AWU_CSR_AWUEN);
 539  0098 721950f0      	bres	20720,#4
 540                     ; 171     AWU->TBR = (uint8_t)(~AWU_TBR_AWUTB);
 542  009c 35f050f2      	mov	20722,#240
 543                     ; 172 }
 546  00a0 81            	ret
 590                     ; 180 FlagStatus AWU_GetFlagStatus(void)
 590                     ; 181 {
 591                     	switch	.text
 592  00a1               _AWU_GetFlagStatus:
 596                     ; 182     return((FlagStatus)(((uint8_t)(AWU->CSR & AWU_CSR_AWUF) == (uint8_t)0x00) ? RESET : SET));
